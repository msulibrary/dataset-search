...
/*
**	AJAX action to get feed content and insert new content into the database
**
**	This file is the same as ../getSimpleFeedContent.php except for hash checking
**	and insertion of records into the database.
*/
...
$feedStructureXml = simplexml_load_file('../feedStructure.xml') or die('feedStructure file not loading');
...
else
{
	...
	foreach ($feedXml->$entryTag as $feedItem)
	{
		...
		// Generate and check hash to look for new datasets

		$entry->status = "new dataset";

		// Create normalized unique string identifier based on feed publisher and extracted data

		$normalizedIdentifier = $feedPublisher;

		foreach (array('title', 'link', 'description', 'creator', 'pubDate', 'uid') as $element)
		{
			if (isset($entry->$element))
			{
				$normalizedIdentifier .= $entry->$element;
			}
		}
		...
		// Replace & with and
		$normalizedIdentifier = str_replace('&', 'and', $normalizedIdentifier);

		// Replace any non-alphanumeric characters with underscores
		$normalizedIdentifier = preg_replace('/[^a-z0-9]+/i', "_", $normalizedIdentifier);

		// Generate a sha1 hash of the normalized string identifier
		$hash = sha1($normalizedIdentifier);

		// Check for existence of this hash in the database
		$hashCheck = mysql_query("
			SELECT recordInfo_recordIdentifier, status
			FROM datasets
		...
		if (!$hashCheck)
		{
			$entry->status = "hash check failed";
		}
		elseif (mysql_num_rows($hashCheck) == 1)
		{
			$row = mysql_fetch_object($hashCheck);
			$entry->recordId = $row->recordInfo_recordIdentifier;

			if ($row->status == 'u')
			{
				// This is an unprocessed dataset that is already in the database
				$entry->status = "new dataset";
			}
			else
			{
				// This dataset is already in the database and has been processed
				$entry->status = "already in database";
			}
		}
		elseif (mysql_num_rows($hashCheck) == 0)
		{
			// This is an unprocessed dataset that is not in the database
			$entry->status = "new dataset";

			// Store the extracted data in the database with 'unprocessed' status
			$insert = "
				INSERT INTO datasets SET
				dataset_name = \"" . (isset($entry->title) ? addslashes($entry->title) : ""). "\",
				dataset_url = \"" . (isset($entry->link) ? addslashes($entry->link) : "null") . "\",
				dataset_description = \"" . (isset($entry->description) ? addslashes($entry->description) : "null") . "\",
				dataset_temporalCoverage = \"" . (isset($entry->pubDate) ? addslashes($entry->pubDate) : "null") . "\",
				dataset_doi = \"" . (isset($entry->uid) ? addslashes($entry->uid) : "null") . "\",
				dataset_urlHash = \"" . $hash . "\",
				status = \"u\";
			";

			if (!mysql_query($insert))
			{
				$entry->status = "insert dataset failure";
			}
			else
			{
				// Get recordInfo_recordIdentifier
				$entry->recordId = mysql_insert_id();

				if (isset($entry->creator) && ($entry->creator != ''))
				{
					$insertCreator = "
						INSERT INTO creators SET
						recordInfo_recordIdentifier = '" . $entry->recordId .  "',
						creator_name = '" . addslashes($entry->creator) . "'
					";

					if (!mysql_query($insertCreator))
					{
						$entry->status = "insert creator failure";
					}
				}
			}
		}
